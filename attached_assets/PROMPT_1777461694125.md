...
APPEAL_DISCUSSION_TOPIC = 11   (topic diskusi appeal, tempat admin menyetujui/menolak appeal)
...
3. Owner clicks "Join Federation":
   ...
   c. If permissions are sufficient:
      ...
      - If fed_owners is empty, insert { user_id: owner_id } as first Federation Owner.
...
- If permissions OK, perform the same upsert into federated_groups (if already active, reply "Already affiliated."). Set is_active=true. If fed_owners empty, set owner as fed_owners.
...
Only fed owner/admins. Usage: /cban <target> <reason> (reason required). Works in any affiliated group, in the MAIN_GROUP (forum), in the EXEC_GROUP, or in bot PM. The executor does not need to be an admin of the chat. Usage: /cban <target> <reason> (reason required). Works in any affiliated group or PM.
...
- Only fed owner/admins. Usage: /cunban <target>. Works in any affiliated group, in the MAIN_GROUP (forum), in the EXEC_GROUP, or in bot PM. The executor does not need to be an admin of the chat. Usage: /cunban <target>.
- Find active ban by banned_user_id. If none, reply "User is not banned."
- Unban juga dapat terjadi secara otomatis jika appeal disetujui melalui sistem appeal (lihat Feature 8).
- Set is_active = false.
- Log to LOG_CHANNEL:
...
FEATURE 11: FEDERATION STATISTICS (/fedstats, /stats, /fedinfo alternatives)
- /fedstats – everyone.
- Reply (HTML):
...
- Only fed owner (or admins if you design so, but specify owner only to be safe). The bot leaves every active federated group (is_active=true). For each, attempt leave_chat. Set each group's is_active=false after successfully leaving. Count successes and failures.
...
- Show a list of all available commands with their aliases in a readable format, each with a short description (no emoji).

FEATURE 20: MESSAGE EDITING BEHAVIOUR
- Prefer editing existing bot messages instead of sending new ones. Use try-except to ignore "Message is not modified" errors. In conversation flows that use Cancel buttons, edit the same message to indicate cancellation or completion.
